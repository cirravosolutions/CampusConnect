...
const PostCard: React.FC<{ post: Post; onEdit?: (post: Post) => void }> = ({ post, onEdit }) => {
    ...
    const { getCommentsForPost, deleteCommentsForPost } = useComments();
    const { admins } = useAdmins();
    const { getPollForPost, deletePollForPost } = usePolls();
    ...
    const handleDelete = async () => {
        if (window.confirm(`Are you sure you want to permanently delete the post "${post.title}"? This will also delete all associated comments and polls.`)) {
            setIsDeleting(true);
            try {
                // Since there is no backend, we manually cascade deletes in the client state.
                await deletePost(post.id);
                if (pollForPost) {
                    await deletePollForPost(post.id);
                }
